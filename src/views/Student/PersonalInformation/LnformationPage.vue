  ...
  <div>
    ...
    <div style="
      ...
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/stu/practice' }">首页</el-breadcrumb-item>
    ...
    <div class="back">
      ...
      <div class="back2">
        <table class="t1" style="border-collapse: collapse; width: 100%">
          <!-- 表格内容 -->
          <tr class="r1">
            <td class="d1"><i class="el-icon-user"></i>学号</td>
            <td class="d1">{{ Data.studentNo }}</td>
            <td class="d1"><i class="el-icon-user-solid"></i>姓名</td>
            <td class="d1">{{ Data.studentName }}</td>
            <td class="d1"><i class="el-icon-aim"></i>年龄</td>
            <td class="d1">{{ Data.studentAge }}</td>
          </tr>
          <tr class="r1">
            <td class="d1">
              <i v-if="Data.sex === '男'" class="el-icon-male"></i>
              <i v-else class="el-icon-female"></i>性别
            </td>
            <td class="d1">{{ Data.studentSex }}</td>
            <td class="d1"><i class="el-icon-aim"></i>入学日期</td>
            <td class="d1">{{ Data.enrolTime }}</td>
            <td class="d1"><i class="el-icon-aim"></i>毕业日期</td>
            <td class="d1">{{ Data.graduateTime }}</td>
          </tr>
          <tr class="r1">
  ...
  methods: {
    ...
    getData() {
      axios.get("/main/selectStudent", { params: { studentNo: localStorage.getItem('userName') } }).then(res => {
        ...
        const et = this.Data.enrolTime.indexOf('T')
        // = this.postList[index].entryTime;
        if (et != -1) {
          let date = new Date(this.Data.enrolTime)
          date.setDate(date.getDate() + 1)
          let newDateStr = date.toISOString().slice(0, et);
          this.Data.enrolTime = newDateStr

        }
        const gt = this.Data.graduateTime.indexOf('T')
        // = this.postList[index].entryTime;
        if (gt != -1) {
          let date = new Date(this.Data.graduateTime)
          date.setDate(date.getDate() + 1)
          let newDateStr = date.toISOString().slice(0, gt);
          this.Data.graduateTime = newDateStr

        }

        this.getCollege(this.Data.collegeID)
        this.getClassName(this.Data.classID)
        this.getSpeciality(this.Data.specialityID)
