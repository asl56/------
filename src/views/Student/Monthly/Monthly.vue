<template>
  <div>
    <div class="MonthlyBox1">
          ...
          >
            ...
            <img src="../../../assets/月报.jpg" class="image" />
              ...
              <div style="padding: 18px">
                <span v-if="item.monthlyReportTitle">{{
                  item.monthlyReportTitle
                }}</span>
                <span v-else
      ...
      <!-- 添加月报弹窗 -->
      <el-dialog :visible.sync="showAddDialogData"
       title="添加月报" 
       width="50%">
        <!-- 弹窗内容 -->
        <el-form :model="addForm" ref="addForm" label-width="80px">
          <el-form-item label="标题">
            <el-input v-model="addForm.monthlyReportTitle"
            @input="handleInput"
          ...
          <el-form-item label="内容">
            <el-input
              type="textarea"
              v-model="addForm.monthlyReportDetails"
              @input="handleInput"
              :disabled="inputDisabled"
            ></el-input>
          </el-form-item>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="saveAddMonthly()">确 定</el-button>
        </span>
      </el-dialog>
      ...
      >
        <div v-if="!editForm.textName">
          ...
          <el-form :model="editForm" ref="editForm" label-width="80px">
            <el-form-item label="标题">
              <el-input v-model="editForm.monthlyReportTitle"></el-input>
            </el-form-item>
            <el-form-item label="内容">
              <el-input
                type="textarea"
                v-model="editForm.monthlyReportDetails"
              ></el-input>
            </el-form-item>
          </el-form>
          <span slot="footer" class="dialog-footer">
      ...
      >
        ...
        <div v-if="!detailForm.textName">
          ...
          <el-form :model="detailForm" label-width="80px">
            <el-form-item label="标题">
              <span>{{ detailForm.monthlyReportTitle }}</span>
            </el-form-item>
            <el-form-item label="内容">
              <span>{{ detailForm.monthlyReportDetails }}</span>
            </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleInput() {
      // 监听输入框输入事件
      if (
        this.addForm.monthlyReportTitle !== "" ||
        this.addForm.monthlyReportDetails !== ""
      ) {
        // 如果标题输入框内容不为空
    ...
    cancelAddDialog() {
      this.clearFiles();
      // this.addForm.monthlyReportTitle = ""; // 清空标题输入框内容
      // this.addForm.monthlyReportDetails = ""; // 清空内容输入框内容
    ...
    handleAvatarSuccess(file, fileList) {
      ...
      console.log(this.addForm.textName);
      this.addForm.monthlyReportTitle = this.addForm.textName;
      if (file.data != "") {
    ...
    searchData111() {
      ...
      axios
        .get("/main/selectMonthlyReport", {
          params: {
            MonthlyReportTitle: this.input,
            createTime: data,
            page: this.page.page,
    ...
    saveAddMonthly() {
      ...
      axios
        .post("/main/insertMonthlyReport", JSON.stringify(this.addForm), {
          headers: {
    ...
    showEditDialog(item) {
      ...
      // this.editForm.monthlyReportTitle = item.monthlyReportTitle;
      // this.editForm.monthlyReportDetails = item.monthlyReportDetails;
      // this.editForm.studentNo = item.studentNo;
      // this.editForm.createTime = this.formatDate(item.createTime);
      // this.editForm.updateTime = this.formatDate(item.updateTime);
      // this.editForm.score = item.score;
      // this.editForm.textName = item.textName;
    ...
    saveEditMonthly() {
      ...
      //this.editForm.monthlyReportTitle = this.addForm.textName;
      console.log(this.editForm);
      axios
        .post("/main/updateMonthlyReport", JSON.stringify(this.editForm), {
          headers: {
            "Content-Type": "application/json",
          },
        })
    ...
    showDetailDialog(item) {
      ...
      // 填充查看详细表单数据
      this.detailForm.monthlyReportTitle = item.monthlyReportTitle;
      this.detailForm.monthlyReportDetails = item.monthlyReportDetails;
...
.MonthlyTp1 {
  width: 100px;
  height: 50px;
  /* background-image: url("../img/YUE.png"); 这里设置背景图片的路径 */
